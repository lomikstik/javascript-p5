...
function setup() {
    ...
    frameRate(60);
//    randomSeed(1234);
    sliders[0] = createSlider(0,10,0,0.5);
...
//            }
    ...
    this.split = function () {
        ...
        this.child.x = this.x;
        this.child.y = this.y;
        this.child.speed = this.speed;  //0.1
        this.child.rotspeed = this.rotspeed;//0.1
        this.child.eyes[0].offset = this.eyes[0].offset;//1
        this.child.eyes[1].offset = this.eyes[1].offset;//1
        this.child.eyes[0].rotoffset = this.eyes[0].rotoffset;//2
        this.child.eyes[1].rotoffset = this.eyes[1].rotoffset;//2
        this.child.fovs[0].range = this.fovs[0].range;//1
        this.child.fovs[0].centerAngle = this.fovs[0].centerAngle;//1
        this.child.fovs[0].width = this.fovs[0].width;//1
        this.child.fovs[1].range = this.fovs[1].range;//1
        this.child.fovs[1].centerAngle = this.fovs[1].centerAngle;//1
        this.child.fovs[1].width = this.fovs[1].width;//1
        this.child.fovs[2].range = this.fovs[2].range;//1
        this.child.fovs[2].centerAngle = this.fovs[2].centerAngle;//1
        this.child.fovs[2].width = this.fovs[2].width;//1
        this.child.r = this.r;//15
        this.child.g = this.g;//15
        this.child.b = this.b;//15
        this.child.rot = this.rot + 180;
        this.child.randomTurnFactor = this.randomTurnFactor;//0.015
        this.child.sprintFactor = this.sprintFactor;//0.05
        this.child.sprintTurnFactor = this.sprintTurnFactor;//0.05
        this.child.splitMass = this.splitMass;//2
        
        //RANDOMIZE THA CHILD!!!!!!!
        if(random(100) < mutationFactor)this.child.speed += random(-0.2,0.2);  //0.1
        if(random(100) < mutationFactor)this.child.rotspeed += random(-0.2,0.2);//0.1
        if(random(100) < mutationFactor)this.child.eyes[0].offset += random(-2,2);//1
        if(random(100) < mutationFactor)this.child.eyes[1].offset += random(-2,2);//1
        if(random(100) < mutationFactor)this.child.eyes[0].rotoffset += random(-4,4);//2
        if(random(100) < mutationFactor)this.child.eyes[1].rotoffset += random(-4,4);//2
        if(random(100) < mutationFactor)this.child.fovs[0].range += random(-2,2);//1
        if(random(100) < mutationFactor)this.child.fovs[0].centerAngle += random(-2,2);//1
        if(random(100) < mutationFactor)this.child.fovs[0].width += random(-2,2);//1
        if(random(100) < mutationFactor)this.child.fovs[1].range += random(-2,2);//1
        if(random(100) < mutationFactor)this.child.fovs[1].centerAngle += random(-2,2);//1
        if(random(100) < mutationFactor)this.child.fovs[1].width += random(-2,2);//1
        if(random(100) < mutationFactor)this.child.fovs[2].range += random(-2,2);//1
        if(random(100) < mutationFactor)this.child.fovs[2].centerAngle += random(-2,2);//1
        if(random(100) < mutationFactor)this.child.fovs[2].width += random(-2,2);//1
        if(random(100) < mutationFactor)this.child.r += random(-45,45);//15
        this.child.r = Math.max(Math.min(this.child.r,255),0);
        if(random(100) < mutationFactor)this.child.g += random(-45,45);//15
        this.child.g = Math.max(Math.min(this.child.g,255),0);
        if(random(100) < mutationFactor)this.child.b += random(-45,45);//15
        this.child.b = Math.max(Math.min(this.child.b,255),0);
        if(random(100) < mutationFactor)this.child.randomTurnFactor += random(-0.05,0.05);//0.015
        if(random(100) < mutationFactor)this.child.sprintFactor += random(-0.1,0.1);//0.05
        if(random(100) < mutationFactor)this.child.sprintTurnFactor += random(-0.1,0.1);//0.05
        if(random(100) < mutationFactor)this.child.splitMass += random(-15,15);//5
...
function FOV(owner) {
    ...
    this.see = function () {
        ...
        for (var kaqs = 0; kaqs < foods.length; kaqs++) {
            if (Math.abs(this.owner.x - foods[kaqs].x) < this.range * worldScale && 
                Math.abs(this.owner.y - foods[kaqs].y) < this.range * worldScale) {
