...
//    cubes[3].h = 40;
    ...
    for (i = 0; i < foodCount; i++){
        foods[i] = new Food();
        foods[i].x = random(border + foods[i].size, 600 + border - foods[i].size);
...
//    
    ...
    if (frameCount % 20 == 0 && foods.length < foodCount) {
        ...
        newFood.x = random(border + newFood.size, 600 + border - newFood.size);
    ...
    if (!mouseIsPressed) {
        ...
        //    }
        //    
            if ((kaqs == 19 || kaqs == 39 || kaqs == 59) && foods.length < foodCount) {
                var newFood = new Food();
                newFood.x = random(border + newFood.size, 600 + border - newFood.size);
                newFood.y = random(border + newFood.size, 600 + border - newFood.size);
                foods.push(newFood);
...
function Caalis() {
    ...
    this.move = function (spd) {
        ...
        this.energy-=Math.pow(spd,2) * 0.01;
    }
